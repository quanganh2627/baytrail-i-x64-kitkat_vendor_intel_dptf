...
void PowerControlFacade::initializeControlsIfNeeded()
{
    if (supportsPowerControls())
    {
        m_policyServices.messageLogging->writeMessageDebug(PolicyMessage(FLF, "Power control initialization started."));
        const PowerControlDynamicCapsSet& caps = getCapabilities();
        ...
        m_policyServices.messageLogging->writeMessageDebug(PolicyMessage(FLF, "Power control initialization finished."));
    }
}